...
interface Repo {
    ...
    fun getuserById(uid: String): Flow<ResultState<UserDataParent>>//Получает пользователя по ID  из Firebase
    ...
    fun getProductsInLimited(): Flow<ResultState<List<ProductDataModels>>>//Получает ограниченное количество продуктов
    ...
    fun getCart(): Flow<ResultState<List<CartDataModels>>>//Получает товары в корзине
